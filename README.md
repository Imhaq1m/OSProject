...
1. What is the link of the fork OSProject in your repository. ***(1 mark)*** (https://github.com/Imhaq1m/OSProject).
2. How many files and folders are in this repository. ***(1 mark)*** There is __one__ folder and there are __seven__ files.
...
__Option 1: low ram, small disk space, and few vcpu for small projects__
__Option 2: high ram, large disk space, and more vcpu for large projects__
3. Why must we commit and sync our current work on source control? ***(1 mark)*** __Commit and sync our current work acts as the backup__.
...
overlay         32847680 10705868  20447712  35% /
...
total 44
 4 drwxrwxrwx+ 4 codespace root  4096 Jan 26 01:02 .
...
11. What is the available free memory in the system. ***(1 mark)*** __308.8 MiB__.
12. What is the available disk space mounted on /workspace. ***(1 mark)*** __20447712__.
...
14. What is the difference between **ls** vs **ls -asl**. ***(1 mark)*** __ls: provides a basic list of files and directories, ls -asl: provides a more detailed and sorted list__.
15. What is the TLB size of the Virtual CPU. ***(1 mark)*** __2560 4K pages__.
16. What is the CPU speed of the Virtual CPU. ***(1 mark)*** __3021.761 MHz__.
17. What is the top running process that consumes the most CPU cycles. ***(1 mark)*** __2547 codespa+__.
...
1. Are files in the container persistent. Why not?. ***(1 mark)*** __They are not persistent. Docker containers are ephemeral, meaning any data created within them is lost when the container is removed__.
...
1. Check the permission of the files created in myroot, what user and group is the files created in docker container on the host virtual machine? . ***(2 mark)*** __-rw-rw-rw- 1 root root 0 Jan 26 02:52 test_file__.
...
```
*** __-rw-rw-rw- 1 codespace codespace 0 Jan 26 02:52 test_file__.***
...
1. What is the permission of folder /usr/local/apache/htdocs and what user and group owns the folder? . ***(2 mark)*** __drwxrwxrwx+ 2 1000 1000 4096 Jan 26 11:31 /usr/local/apache2/htdocs__.
...
1. Describe what is busybox and what is command switch **--name** is for? . ***(2 mark)*** __BusyBox is a lightweight, open-source software suite that combines tiny versions of many common UNIX utilities into a single executable. --name command is for Docker commands to assign a name to a container when running it__.
2. Explore the network using the command ```docker network ls```, show the output of your terminal. ***(1 mark)*** __
NETWORK ID     NAME      DRIVER    SCOPE
8d667633554e   bluenet   bridge    local
415e976ffb0c   bridge    bridge    local
19f437de3be6   host      host      local
4365f58f88ee   none      null      local
bba5cfed6c89   rednet    bridge    local__.
...
4. What is the network address for the running container c1 and c2? ***(1 mark)*** __172.18.0.0__.
...
1. Are you able to ping? Show your output . ***(1 mark)*** __
...
64 bytes from 172.20.0.3: seq=5 ttl=64 time=0.149 ms
